...
u_int mktcbid(struct Tcb *t) {
    ...
    u_int tcb_num = ((u_int)t - (u_int)e - BY2PG / 2) / 256;
    return ((e->env_id << 3) | tcb_num);
}
...
int thread_alloc(struct Env *e, struct Tcb **new) {
    ...
    u_int thread_num = e->env_thread_count;
    u_int i = 0;
    while (e->env_threads[thread_num].tcb_status != ENV_FREE) {
        thread_num++;
        thread_num = thread_num % THREAD_MAX;
        i++;
        if (i >= THREAD_MAX) {
            return E_THREAD_MAX;
        }
    ...
    e->env_thread_count++;
    struct Tcb *t = &e->env_threads[thread_num];
    t->tcb_id = mktcbid(t);
    printf("the new thread id is %x\n", t->tcb_id);
    t->tcb_status = ENV_RUNNABLE;
    t->tcb_exit_ptr = (void *)0;
    t->tcb_tf.cp0_status = 0x1000100c;
    t->tcb_tf.regs[29] = USTACKTOP - 4 * BY2PG * (t->tcb_id & 0x7);
    t->tcb_cancel_state = THREAD_CANNOT_CANCEL;
    t->tcb_cancel_type = THREAD_CANCEL_ASYN;
    t->tcb_canceled = 0;
    t->tcb_exit_value = 0;
...
env_destroy(struct Env *e)
{
    ...
    if (curenv == e) {
        ...
        printf("i am an env ... i am killed ... \n");
        sched_yield();
    }
}
...
void thread_destroy(struct Tcb *t) {
    ...
    if (curtcb == t) {
        ...
        bcopy((void *)KERNEL_SP - sizeof(struct Trapframe),
              (void *)TIMESTACK - sizeof(struct Trapframe),
              sizeof(struct Trapframe));
        printf("i am a thread ... i am killed ...\n");
        sched_yield();
    }
}
